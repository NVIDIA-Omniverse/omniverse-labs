...
I used GPT-6 Astra in Codex \+ NVIDIA Omniverse libraries to build, test, and debug the simulation directly in VS Code.

Here’s what I learned along the way: \[LINK\]
...
[Astra announcement](https://openai.com/index/gpt-6-astra/)
...
The existing extension built from another project using `ovrtx` and HTTP service gave us a front end to build on. A separate Python worker runs the physics at a fixed timestep and sends the measured state, labeled by tick, to the viewer. `ovstage` applies those transforms, and `ovrtx` renders them. The same runner and saved JSON work with or without the editor open.

![Figure 1. How the pieces fit together. The editor and headless runs share a runner and saved configuration.](images/figure-1-architecture.png)
...
For the conveyor and box, we used Conveyor\_Belt\_A61, Box\_Cube\_A01, and their selected dependencies from the SimReady Warehouse dataset. That came to 19 files, about 76.6 MB. Much more manageable than downloading a warehouse to test one box. 

[NVIDIA Skills catalog](https://build.nvidia.com/skills)

[CAD-to-SimReady Skill](https://github.com/NVIDIA/skills/tree/main/skills/omniverse-cad-to-simready)

[Physical AI SimReady Warehouse catalog](https://huggingface.co/datasets/nvidia/PhysicalAI-SimReady-Warehouse-01)
...
![Figure 3. Same original box, different stopping policy. Continuous motion misses the settling requirement; the indexed stop reaches the required half-second dwell. Time starts when the box is fully supported by the receiver.](images/figure-3-settling.png)

Figure 3\. Same original box, different stopping policy. Continuous motion misses the settling requirement; the indexed stop reaches the required half-second dwell. Time starts when the box is fully supported by the receiver.
...
![Figure 4. Wider gaps still fail. These runs stop at the tilt failure, so the peaks shown are what we measured before termination. The original box is about 105.7 mm long in the direction of travel.](images/figure-4-wider-gaps.png)

Figure 4\. Wider gaps still fail. These runs stop at the tilt failure, so the peaks shown are what we measured before termination. The original box is about 105.7 mm long in the direction of travel.
...
![Figure 6. A separate two-package editor test after the drop. One healthy box went through and one reject settled in the tray. The table above comes from the four-package mixed trial.](images/figure-6-two-package-test.png)

Figure 6\. A separate two-package editor test after the drop. One healthy box went through and one reject settled in the tray. 

![Watch the Robotiq pickup](media/robotiq-pick.webm)

The recording shows a two-package run through the  `ovtrx` viewer at nominal 2× simulation time. It uses native worker snapshots, so you’re watching the simulated motion, not animated timeframes. The four-package report has the totals for all three defect types.
...
\[ovphysx documentation\](https://github.com/NVIDIA-Omniverse/PhysX/tree/main/ovphysx)
...
* [GTC Berlin vision AI sessions](https://www.nvidia.com/en-eu/gtc/sessions/building-vision-ai-agents/)

Omniverse Simulation Libraries

* [ovphysx library](https://github.com/NVIDIA-Omniverse/PhysX/tree/main/ovphysx)  
* [ovstage library](https://github.com/NVIDIA-Omniverse/ovstage)  
* [ovrtx library](https://github.com/NVIDIA-Omniverse/ovrtx)

OpenUSD Content Libraries
