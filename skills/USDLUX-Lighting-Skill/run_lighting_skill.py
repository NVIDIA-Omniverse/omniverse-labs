...
# Minimum largest bounding-box dimension (m) for a glass prim to count as a window.
# Compared against the longest axis of the world-space bbox — not an area.
# Forklift windshield longest dim ~0.3 m, fire extinguisher glass ~0.1 m.
# A real window is typically >= 0.5 m on its longest axis.
GLASS_MIN_DIM_M = 0.5
...
def is_glass(prim, bbox_cache=None, mpu=1.0):
    ...
    - Glass geometry that is too small to be a window (< GLASS_MIN_DIM_M)
    """
    name = prim.GetName().lower()
    path = str(prim.GetPath()).lower()
    combined = name + " " + path
    ...
    if "window" not in name and bbox_cache is not None:
        try:
            ...
            if dims[2] < GLASS_MIN_DIM_M:  # largest dimension < 0.5 m → too small
                return False
        except Exception:
...
def select_best_submesh_positions(raw_positions):
    ...
    for parent_path, entries in groups.items():
        ...
        if has_geo:
            best = has_geo
        # Tertiary tiebreaker: flat-panel detection is the highest-confidence result.
        # If any entry came from flat-panel analysis, discard all non-flat-panel entries.
        has_flat = [e for e in best if isinstance(e[4], dict) and e[4].get('from_flat_panel')]
        if has_flat:
            best = has_flat
            # Flat-panel positions must not be over-merged — each represents a distinct panel.
            best = deduplicate_positions(best, threshold_m=0.15)
        else:
            # Decide merge threshold based on whether sub-mesh leaf names carry fixture keywords.
            # If names are generic (e.g. "Section0", "Section1" detected via ancestor "lamp"):
            #   → large threshold so housing and emitter sub-meshes collapse to one light.
            # If names contain fixture keywords (e.g. "led01", "led02", "led03"):
            #   → small threshold so each discrete fixture instance is kept separate.
            _all_kw = EXPLICIT_FIXTURE_KEYWORDS + FIXTURE_KEYWORDS
            _has_named = any(
                any(k in e[3].lower() for k in _all_kw)
                for e in best
            )
            _thresh = 0.15 if _has_named else 1.0
            best = deduplicate_positions(best, threshold_m=_thresh)
        final.extend(best)
...
def _panel_normal_to_euler_xyz(nx, ny, nz):
    """
    Convert a unit panel normal (emission direction) to Euler XYZ rotation degrees
    for a UsdLux light whose default emission is along local -Z.

    USD xformOp:rotateXYZ uses extrinsic X-Y-Z order, i.e. matrix = R_z(z)·R_y(y)·R_x(x).
    With x always 0, the emission direction of a rotated -Z vector is:
        n = (-sin y · cos z,  -sin y · sin z,  -cos y)
    Solving for y and z given (nx, ny, nz):
      • when nx > 0 (or nx≈0 and ny > 0): choose y < 0
            y = -arccos(-nz),  z = atan2(ny, nx)
      • otherwise: choose y > 0
            y = +arccos(-nz),  z = atan2(-ny, -nx)
    Verified against all five user-provided ground-truth orientations.
    """
    if abs(nz + 1.0) < 1e-6:      # already facing -Z: default orientation
        return 0.0, 0.0, 0.0
    if abs(nz - 1.0) < 1e-6:      # facing +Z: flip 180° around Y
        return 0.0, 180.0, 0.0
    _cos_y = max(-1.0, min(1.0, -nz))
    if nx > 1e-6 or (abs(nx) <= 1e-6 and ny > 1e-6):
        # y < 0 branch: n = (sin|y|·cos z, sin|y|·sin z, -cos y)
        rot_y = -math.degrees(math.acos(_cos_y))
        rot_z = math.degrees(math.atan2(ny, nx))
    else:
        # y > 0 branch: n = (-sin y·cos z, -sin y·sin z, -cos y)
        rot_y = math.degrees(math.acos(_cos_y))
        rot_z = math.degrees(math.atan2(-ny, -nx))
    return 0.0, rot_y, rot_z
...
def find_flat_panels_in_mesh(mesh, mpu, world_xform, angle_deg=25.0,
    ...
    for actual_m2, face_list, tot_a, anx, any_, anz in selected:
        ...
        dim0_m   = max(span_u_m + margin_m, 0.05) * 0.9
        dim1_m   = max(span_v_m + margin_m, 0.05) * 0.9

        # Offset centroid outward along the panel normal so the RectLight clears the geometry.
        # Scale with panel size: sqrt(face area in scene units) × 0.17 gives ~10 cm for a
        # 3900 cm² panel (mpu=0.01), matching the empirical offset from the modular.usd run.
        _off_su = math.sqrt(tot_a) * 0.17
        panels.append((
            wc[0] + wn[0] * _off_su,
            wc[1] + wn[1] * _off_su,
            wc[2] + wn[2] * _off_su,
...
def check_glass_geometry(stage, bbox_cache, mpu):
    ...
    3. Minimum size threshold — glass must be >= GLASS_MIN_DIM_M to count as a window
                        ...
                        if "window" in prim_name_lower:
                            ...
                            try:
                                ...
                                dims = sorted([abs(size[0]) * mpu, abs(size[1]) * mpu,
                                               abs(size[2]) * mpu])
                                if dims[2] >= GLASS_MIN_DIM_M:  # largest dim >= 0.5 m
                                    glass_prims.append(prim)
                            except Exception:
                                pass
            except Exception:
...
def apply_dome_light_schemas(prim, illuminance=10000.0, color_temp=6500.0,
                             set_version=True, normalize_intensity=True):
    """Apply PhotometricDomeLightAPI + PhysicalLightIlluminantAPI for dome lights.

    set_version:        always set omni:rtx:usdluxVersion = 2505.
    normalize_intensity: set inputs:intensity=1.0 / inputs:exposure=0.0 so the
                         photometric:illuminance value is the sole brightness driver.
                         Pass False when upgrading an existing dome whose base layer
                         already has inputs:intensity set (e.g. 1000.0 for an HDR sky)
                         — writing 1.0 would dim it 1000×.
    """
...
def run_skill(stage,
    ...
    _tc_vis = Usd.TimeCode.Default()
    for fp in fixtures_prims:
        # Skip fixtures that are invisible in the composed scene — their ancestor
        # instance may have visibility=invisible, which ComputeVisibility inherits.
        if UsdGeom.Imageable(fp).ComputeVisibility(_tc_vis) == UsdGeom.Tokens.invisible:
            log(f"  Skipping invisible fixture: {fp.GetPath()}")
            continue
    ...
    if force_dome or has_glass:
        _eff_dome_illum = dome_illuminance if dome_illuminance > 0 else 15000.0
    ...
    if has_glass:
        if dome_illuminance <= 0:
            dome_illuminance = 15000.0
            log(f"  (Dome illuminance auto: {dome_illuminance} lux — overcast midday)")
    elif force_dome and dome_illuminance <= 0:
        dome_illuminance = 15000.0
        log(f"  (Dome illuminance auto: {dome_illuminance} lux — overcast midday, forced)")
    ...
    for i, fixture_entry in enumerate(all_fixture_positions):
        ...
        if dry_run:
            ...
            # Flat-panel RectLights: orient to face the world-space panel normal.
            # Uses xformOp:rotateXYZ (extrinsic, matrix = R_z·R_y·R_x, x always 0).
            if (light_type == "RectLight" and fmeta and fmeta.get('from_flat_panel')
                    and fmeta.get('panel_normal') is not None):
                _pn = fmeta['panel_normal']
                _, _ry, _rz = _panel_normal_to_euler_xyz(_pn[0], _pn[1], _pn[2])
                xform.AddRotateXYZOp().Set(Gf.Vec3f(0.0, float(_ry), float(_rz)))
            # Rotate cluster panel RectLights to match the lamp assembly's world orientation.
            ...
            elif (light_type == "RectLight" and fmeta and fmeta.get('from_cluster')
    ...
    #   * If one already exists → upgrade in-place to 2505 with rotation compensation
    #   * If none exists → create a new one at 2505
    # - no glass, no force → no DomeLight needed
    #   * If existing DomeLights present → deactivate them (orphaned)
    #
    # 2411→2505 rotation compensation:
    #   In 2411 the DomeLight zenith defaults to +Z local; the common (270,0,0)
    #   rotation corrects that to world +Y for Y-up scenes.
    #   In 2505 the zenith is already +Y local, so that correction is redundant.
    #   Net offset: add +90° to the existing X rotation ((rx+90)%360, ry, rz).
    _DOME_RX_COMP = 90.0
    ...
    if need_dome:
        # Read appearance attrs from the first existing dome BEFORE deactivating it.
        # Read rotation from the base layer specifically (not the composed stage) so
        # re-running the skill doesn't accumulate the compensation offset.
        _dome_tex_file = _dome_tex_fmt = _dome_color = _dome_vis_pr = None
        _dome_base_rot = None
        _dome_rot_op_type = None  # 'XYZ' or 'ZYX' — determines component layout
        _dome_mat_binding = None  # material:binding targets (procedural sky MDL etc.)
        if existing_dome_lights:
            _src = existing_dome_lights[0]
            def _ga(n): a = _src.GetAttribute(n); return a.Get() if a.IsValid() else None
            _dome_tex_file = _ga("inputs:texture:file")
            _dome_tex_fmt  = _ga("inputs:texture:format")
            _dome_color    = _ga("inputs:color")
            _dome_vis_pr   = _ga("visibleInPrimaryRay")
            # Copy material:binding (procedural sky MDL shaders use this instead of texture:file)
            _mb_rel = _src.GetRelationship("material:binding")
            if _mb_rel.IsValid():
                _mb_tgts = _mb_rel.GetTargets()
                if _mb_tgts:
                    _dome_mat_binding = list(_mb_tgts)
            # Read rotation from the source layer (exclude our own edit target).
            # rotateXYZ stores (X,Y,Z); rotateZYX stores (Z,Y,X) — track op type for correct mapping.
            _edit_id = stage.GetEditTarget().GetLayer().identifier
            for _lyr in stage.GetLayerStack():
                if _lyr.identifier == _edit_id:
                    continue
                _sp = _lyr.GetPrimAtPath(str(_src.GetPath()))
                if _sp:
                    if "xformOp:rotateXYZ" in _sp.attributes:
                        _dome_base_rot = _sp.attributes["xformOp:rotateXYZ"].default
                        _dome_rot_op_type = 'XYZ'
                        break
                    elif "xformOp:rotateZYX" in _sp.attributes:
                        _dome_base_rot = _sp.attributes["xformOp:rotateZYX"].default
                        _dome_rot_op_type = 'ZYX'
                        break
                    elif "xformOp:orient" in _sp.attributes:
                        # Quaternion (w, xi, yj, zk) → Euler XYZ degrees
                        _q  = _sp.attributes["xformOp:orient"].default
                        _qw = _q.GetReal()
                        _qi = _q.GetImaginary()
                        _qx, _qy, _qz = _qi[0], _qi[1], _qi[2]
                        _rx = math.degrees(math.atan2(
                            2*(_qw*_qx + _qy*_qz), 1 - 2*(_qx*_qx + _qy*_qy)))
                        _ry = math.degrees(math.asin(
                            max(-1.0, min(1.0, 2*(_qw*_qy - _qz*_qx)))))
                        _rz = math.degrees(math.atan2(
                            2*(_qw*_qz + _qx*_qy), 1 - 2*(_qy*_qy + _qz*_qz)))
                        _dome_base_rot = (_rx, _ry, _rz)
                        _dome_rot_op_type = 'XYZ'  # already decoded to XYZ Euler
                        break

        # Deactivate all existing domes — new one replaces them.
        for dome_lp in existing_dome_lights:
            if dry_run:
                log(f"  [DRY RUN] DEACTIVATE old DomeLight: {dome_lp.GetPath()}")
            else:
                _deactivate_and_hide(dome_lp)
                log(f"  Deactivated old DomeLight: {dome_lp.GetPath()}")

        dome_path = "/World/Lights/Environment_DomeLight"
        if dry_run:
            _rot_str = ""
            if _dome_base_rot is not None:
                if _dome_rot_op_type == 'ZYX':
                    _rx, _ry, _rz = _dome_base_rot[2], _dome_base_rot[1], _dome_base_rot[0]
                else:
                    _rx, _ry, _rz = _dome_base_rot[0], _dome_base_rot[1], _dome_base_rot[2]
                _crx = (_rx + _DOME_RX_COMP) % 360.0
                _rot_str = (f"  rotation [{_dome_rot_op_type}] (x={_rx:.0f}…)"
                            f" → rotateXYZ ({_crx:.0f},{_ry:.0f},{_rz:.0f})")
            log(f"  [DRY RUN] Would create DomeLight (2505) at "
                f"{dome_illuminance} lux, {dome_color_temp}K{_rot_str}")
            dome_created = True
        else:
            dome = UsdLux.DomeLight.Define(stage, dome_path)
            dp   = dome.GetPrim()

            # Copy appearance from old dome.
            if _dome_tex_file is not None:
                dp.CreateAttribute("inputs:texture:file",
                                   Sdf.ValueTypeNames.Asset).Set(_dome_tex_file)
            if _dome_tex_fmt is not None:
                dp.CreateAttribute("inputs:texture:format",
                                   Sdf.ValueTypeNames.Token).Set(_dome_tex_fmt)
            if _dome_color is not None:
                dp.CreateAttribute("inputs:color",
                                   Sdf.ValueTypeNames.Color3f).Set(_dome_color)
            if _dome_vis_pr is not None:
                dp.CreateAttribute("visibleInPrimaryRay",
                                   Sdf.ValueTypeNames.Bool).Set(_dome_vis_pr)
            # Copy material:binding — used by procedural sky MDL shaders instead of texture:file.
            if _dome_mat_binding:
                dp.CreateRelationship("material:binding").SetTargets(_dome_mat_binding)
                log(f"  Copied material:binding -> {_dome_mat_binding[0]}")

            # Apply compensated rotation: base-layer rotation + 90° X offset
            # converts the 2411 convention (zenith=+Z) to 2505 (zenith=+Y).
            # rotateXYZ Vec3=(X,Y,Z); rotateZYX Vec3=(Z,Y,X); orient quaternion already decoded above.
            if _dome_base_rot is not None:
                if _dome_rot_op_type == 'ZYX':
                    _rx, _ry, _rz = _dome_base_rot[2], _dome_base_rot[1], _dome_base_rot[0]
                else:
                    _rx, _ry, _rz = _dome_base_rot[0], _dome_base_rot[1], _dome_base_rot[2]
                _crx = (_rx + _DOME_RX_COMP) % 360.0
                UsdGeom.Xformable(dp).AddRotateXYZOp().Set(
                    Gf.Vec3f(float(_crx), float(_ry), float(_rz)))
                log(f"  rotation [{_dome_rot_op_type}] (x={_rx:.0f},y={_ry:.0f},z={_rz:.0f})"
                    f" → rotateXYZ ({_crx:.0f},{_ry:.0f},{_rz:.0f})")

            apply_dome_light_schemas(dp, illuminance=dome_illuminance,
                                     color_temp=dome_color_temp,
                                     set_version=True, normalize_intensity=True)
            src = f" (from {existing_dome_lights[0].GetPath()})" if existing_dome_lights else ""
            log(f"  Created DomeLight (2505): {dome_path} "
                f"({dome_illuminance} lux, {dome_color_temp}K){src}")
            dome_created = True
    else:
        if existing_dome_lights:
