...
def _stage_metrics(stage) -> dict:
    """Return composed physical stage metadata needed by overlay root layers."""
    try:
        ...
        print(
            f"[parse_fmi_schema] WARNING: could not read stage metrics: {exc}",
            file=sys.stderr,
        )
        return {
            "up_axis": "Y",
...
def _world_up(stage) -> dict:
    ...
    for prim in stage.Traverse():
        ...
        try:
            ...
            continue
        # UsdPhysicsScene defines a zero gravity direction as a request to use
        # negative stage upAxis. It is not an authored Y-down direction.
        if math.sqrt(sum(component * component for component in direction)) < 1e-8:
            continue
        up = _normalised_vec3(
...
def parse(usd_path: str) -> dict:
        ...
        if prim.GetName().lower().startswith("sensor"):
            ...
            if position is not None:
                ...
                overlap_sensors[sensor_path] = {
                    ...
                    "radius": radius,
                    "proxies": proxies,
                    "query_shape": proxies[0] if proxies else None,
                }
    ...
    return {
        ...
        "stage_metrics": _stage_metrics(stage),
    }
