...
def test_first_person_controller_moves_forward_in_camera_space():
    ...
    updated = controller.update(0.5)

    assert updated is not None
    np.testing.assert_allclose(updated[0, 3, 0:3], [0.0, 0.0, -1.0])
...
def test_z_up_horizontal_mouselook_yaws_around_world_up():
    ...
    updated_batch = controller.update(0.0)
...
def test_preview_overlay_authors_render_product_without_touching_source(tmp_path):
    ...
    assert "kilogramsPerUnit = 2" in usda
    assert 'def Camera "OvFmiPreviewCamera"' in usda
    assert "float exposure:fStop = 5" in usda
    assert "float exposure:responsivity = 1.1026709" in usda
    assert "float exposure:time = 0.02" in usda
...
def test_preview_overlay_stage_metrics_ignore_viewer_up_override(tmp_path):
    source = tmp_path / "z_up.usda"
    source.write_text("#usda 1.0\n")
    usda = _make_preview_overlay_usda(
        str(source),
        "/Render/Camera",
    ...
    assert 'upAxis = "Z"' in usda
    assert 'upAxis = "Y"' not in usda


def test_schema_parser_preserves_metrics_and_resolves_zero_gravity_from_up_axis(
    tmp_path,
):
    usd_python = os.environ.get("USD_PYTHON")
    if not usd_python or not Path(usd_python).is_file():
        pytest.skip("isolated USD Python is not configured")

    source = tmp_path / "z_up_physics.usda"
    source.write_text(
        """#usda 1.0
(
    metersPerUnit = 1
...
"""
    ...
    parser = _OV_FMI / "parse_fmi_schema.py"
    result = subprocess.run(
        [usd_python, str(parser), str(source)],
        capture_output=True,
        text=True,
        check=True,
    )
    ...
    assert parsed["stage_metrics"] == {
        "up_axis": "Z",
        "meters_per_unit": 1.0,
        "kilograms_per_unit": 2.0,
    }
    assert parsed["world_up"] == {
        "vector": [0.0, 0.0, 1.0],
