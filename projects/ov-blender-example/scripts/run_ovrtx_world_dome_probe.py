...
"""Probe same-session OVRTX updates for Blender World to DomeLight values."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=REPO / "out" / "artifacts" / "ovrtx-world-dome-probe")
    ...
    fixture = _generated_world_dome_probe_fixture(args.output_dir / "world_dome_probe.usda", args.width, args.height)
    worker_command = worker_command_for_port(args.worker_command, args.port) if args.port else args.worker_command
    paths = {
        "result": args.output_dir / "world-dome-runtime.json",
        "setup": args.output_dir / "world_dome_runtime_setup.py",
    ...
    if not paths["result"].exists():
        print(json.dumps({"status": "failed", "result": str(paths["result"]), "blender_log": str(paths["log"]), "error": "Probe did not write world-dome-runtime.json."}, indent=2, sort_keys=True))
        return 1
...
def _generated_world_dome_probe_fixture(path: Path, width: int, height: int) -> dict[str, str]:
...
def _setup_script(config: dict[str, object]) -> str:
    return textwrap.dedent(
        ...
        from ovrtx_blender_example import world_dome_conversion
        ...
        source_world_color = [0.05, 0.2, 1.0]
        source_world_strength = 20.0
        effective_rgb = [max(0.0, channel * source_world_strength) for channel in source_world_color]
        peak = max(effective_rgb)
        dome_color = [channel / peak for channel in effective_rgb]
        dome_intensity = peak * world_dome_conversion.DOME_LIGHT_SCALE
        result = {{
            "schema_version": 1,
            "artifact_id": "ovrtx-world-dome-runtime-probe",
            "status": "running",
            "usd_path": CONFIG["usd_path"],
            "render_product_path": CONFIG["render_product_path"],
            "worker_command": CONFIG["worker_command"],
            "worker_log": CONFIG["worker_log"],
            "target_dome": CONFIG["target_dome"],
            "source_world_color": source_world_color,
            "source_world_strength": source_world_strength,
            "effective_rgb": effective_rgb,
            "peak": peak,
            "dome_light_scale": world_dome_conversion.DOME_LIGHT_SCALE,
            "conversion_policy": world_dome_conversion.DOME_POLICY_VERSION,
            "formula": "effective_rgb -> color=effective_rgb/peak; intensity=peak*DOME_LIGHT_SCALE",
            "scene_linear_rgb_assumption": True,
        ...
        try:
            ...
            simulation_id = client.start_session(ovrtx_session.build_spec(request), simulation_id="ovrtx-world-dome-probe")
            ...
            requested_world_values = [
                {{
                    "prim_path": CONFIG["target_dome"],
                    "attribute": "inputs:intensity",
                    "value": dome_intensity,
                    "value_type": "Float",
                }},
                {{
                    "prim_path": CONFIG["target_dome"],
            ...
            world_values = [
                ...
                for value in requested_world_values
            ]
            update = client.update_attribute_values(simulation_id, world_values)
            ...
            blue_channel_dominant = updated_mean[2] > updated_mean[0] and updated_mean[2] > updated_mean[1]
            result.update(
                {{
                    "status": "pass" if baseline_sha != updated_sha and mean_abs_delta > 1.0 and accepted_count == len(world_values) and blue_channel_dominant else "failed",
                    "session_simulation_id": simulation_id,
                    "session_reused": True,
                    "physics_generation_reset": False,
                    "requested_world_values": requested_world_values,
                    "accepted_world_value_count": accepted_count,
                    "target_usd_attributes": [value["attribute"] for value in requested_world_values],
                    "update": {{
                    ...
                    "blue_channel_dominant": blue_channel_dominant,
                    "mean_abs_delta": mean_abs_delta,
                    "baseline_completed_samples": baseline.completed_samples,
                    "updated_completed_samples": updated.completed_samples,
                    "baseline_session_completed_samples": baseline.session_completed_samples,
                    "updated_session_completed_samples": updated.session_completed_samples,
