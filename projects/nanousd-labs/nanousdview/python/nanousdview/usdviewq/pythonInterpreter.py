...
class Controller(QtCore.QObject):
    ...
    def _DoAutoImports(self):
        try:
            modules = Tf.ScriptModuleLoader().GetModulesDict() or {}
        except Exception:
            modules = {}
        if not hasattr(modules, "items"):
            modules = {}
        for name, mod in modules.items():
            moduleName = getattr(mod, "__name__", None)
            if moduleName:
                self.interpreter.runsource('import ' + moduleName +
                                           ' as ' + name + '\n')
