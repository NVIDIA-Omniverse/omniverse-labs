...
class StateSource(object):
    ...
    def _typeCheck(self, value, prop):
        ...
        if valueType is not prop.propType:
            ...
            elif prop.propType is str and valueType is str:
                pass # str can be used interchangeably.
            else:
...
class ConfigManager:
    ...
    def save(self, newName=None):
        ...
        self.settings.save()
        # Refresh the cached config paths so getConfigs() picks up the new
        # file on the same session — without this the File → Reopen With
        # State / Save State As submenus stay stale until the next launch.
        self._configPaths = self._loadConfigPaths()
