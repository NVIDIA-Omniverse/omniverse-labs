...
import os, time, sys, platform, math, shlex, subprocess
...
def GetNanousdviewCommand(usdPath, extraArgs=()):
    argv0 = sys.argv[0] if sys.argv else ""
    if argv0 and os.path.exists(argv0) and os.access(argv0, os.X_OK):
        cmd = [argv0]
    else:
        cmd = [sys.executable, "-m", "nanousdview"]
    cmd.append(str(usdPath))
    cmd.extend(str(arg) for arg in extraArgs)
    return cmd

def SpawnNanousdview(usdPath, extraArgs=()):
    return subprocess.Popen(
        GetNanousdviewCommand(usdPath, extraArgs), close_fds=True)

def _EditorCommand(editor, filePath):
    parts = shlex.split(editor)
    if not parts:
        return []
    if any("{}" in part for part in parts):
        return [part.format(str(filePath)) for part in parts]
    return parts + [str(filePath)]

def OpenFileInEditor(filePath):
    for envName in ("NANOUSDVIEW_LAYER_EDITOR", "USDVIEW_LAYER_EDITOR",
                    "VISUAL", "EDITOR"):
        editor = os.environ.get(envName)
        if editor:
            cmd = _EditorCommand(editor, filePath)
            if cmd:
                return subprocess.Popen(cmd, close_fds=True)

    if sys.platform == "darwin":
        return subprocess.Popen(["open", "-t", str(filePath)], close_fds=True)
    if sys.platform.startswith("win"):
        os.startfile(str(filePath))  # type: ignore[attr-defined]
        return None
    return subprocess.Popen(["xdg-open", str(filePath)], close_fds=True)
...
def GetShortStringForValue(prop, val):
    ...
    if valType.isArray and not isinstance(val, Sdf.ValueBlock):
        def _fmt_elem(x):
            # Pixar renders vec3 as (x, y, z), not [x y z]. numpy rows
            # stringify as the bracket form, so coerce to tuple first.
            try:
                if hasattr(x, 'tolist'):
                    t = x.tolist()
                    if isinstance(t, list):
                        return '(' + ', '.join(repr(e) for e in t) + ')'
                    return repr(t)
            except Exception: pass
            return str(x)
        def arrayToStr(a):
            from itertools import chain
            elems = list(a if len(a) <= 6 else
                         chain(a[:3], ['...'], a[-3:]))
            return '[' + ', '.join(_fmt_elem(e) for e in elems) + ']'
...
def _RemoveVisibilityRecursive(primSpec):
    try:
        ...
        # Pixar's primSpec.attributes raises IndexError on missing keys; the
        # nanousd shim uses a dict and raises KeyError. Catch both so this
        # works regardless of which backend is providing the spec.
        pass
