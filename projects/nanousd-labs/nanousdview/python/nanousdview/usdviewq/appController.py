...
class AppController(QtCore.QObject):
    ...
    def __init__(self, parserData, resolverContextFn):
        ...
        with self._makeTimer('bring up the UI'):
            ...
            # Pixar's mainWindowUI binds F/W/V/I/J/C as menu-level
            # WindowShortcut, which intercepts these keys before StageView
            # ever sees them. nuView routes these keys to the viewport
            # (frame, fly mode, etc.) so we strip the shortcuts here.
            for _action_name in ("actionFrame_Selected",  # F → frame selected
                                 "actionWatch_Window",   # W → forward
                                 "actionToggle_Framed_View",  # J
                                 "showInterpreter",      # I
                                 "showUsdValidation",    # V
                                 "actionVersion_Info",   # V
                                 "actionAuto_Compute_Clipping_Planes"):  # C
                _a = getattr(self._ui, _action_name, None)
                if _a is not None:
                    try: _a.setShortcut("")
            ...
            if not self._debugFlagsAvailable():
                self._ui.showDebugFlags.setEnabled(False)
                self._ui.showDebugFlags.setToolTip(
                    "No Tf debug symbols are available in the current "
                    "compatibility layer.")

            try:
                from ._usdviewq import HydraObserver  # noqa: F401
            except ImportError:
                self._ui.showHydraSceneDebugger.setEnabled(False)
                self._ui.showHydraSceneDebugger.setToolTip(
                    "Hydra scene debugger requires Hydra observer support.")
            else:
    ...
    def _rendererAovChanged(self, aov):
        if self._stageView:
            if not self._stageView.SetRendererAov(aov):
                self.statusMessage("Renderer AOV unsupported: %s" % aov, 4)
                self._restoreRendererAovSelection()
                return False
            self._ui.aovOtherAction.setText("Other...")
            return True
        return False

    def _restoreRendererAovSelection(self):
        if not self._stageView or not hasattr(self._ui, "rendererAovActionGroup"):
            return
        current_aov = self._stageView.rendererAovName
        for action in self._ui.rendererAovActionGroup.actions():
            if getattr(action, "aov", None) == current_aov:
    ...
    def _configureRendererAovs(self):
        if self._stageView:
            ...
            customAovsSupported = getattr(
                self._stageView, "SupportsCustomRendererAovs", lambda: True)()
            for aov in aovs:
            ...
            self._ui.aovOtherAction.setEnabled(customAovsSupported)
            if not customAovsSupported:
                self._ui.aovOtherAction.setToolTip(
                    "Custom renderer AOVs are not supported by the current "
                    "backend.")
    ...
    def _otherAov(self):
        ...
        if (ok and len(aov) > 0):
            if self._rendererAovChanged(str(aov)):
    ...
    def _resetPrimView(self, restoreSelection=True):
        if os.environ.get("NUSD_VIEW_DEFER_PRIM_BROWSER", "").lower() in (
            "1", "true", "yes", "on"
        ):
            with self._makeTimer("defer Prim Browser population"), BusyContext():
        ...
        try:
            startingDepth = max(1, int(os.environ.get("NUSD_VIEW_PRIM_DEPTH", "3")))
        except ValueError:
            startingDepth = 3
    ...
    def _resetView(self,selectPrim = None):
        ...
        skipInitialFrame = os.environ.get(
            "NUSD_VIEW_SKIP_INITIAL_FRAME", ""
        ...
        skipRootSelection = skipInitialFrame and selectPrim == pRoot
        item = None
        if not skipRootSelection:
        ...
        if self._stageView:
            if skipInitialFrame and selectPrim == pRoot:
                if self._startingPrimCamera is not None:
                    self._dataModel.viewSettings.cameraPrim = self._startingPrimCamera
                self._stageView.updateView()
            elif (selectPrim and selectPrim != pRoot) or not self._startingPrimCamera:
    ...
    def _debugFlagsAvailable(self):
        debug = getattr(Tf, "Debug", None)
        required = (
            "GetDebugSymbolNames",
        ...
        if not debug or not all(callable(getattr(debug, name, None))
                                for name in required):
            return False
        try:
            return bool(debug.GetDebugSymbolNames())
        except Exception:
            return False

    def _showDebugFlags(self):
        if not self._debugFlagsAvailable():
            self.statusMessage("No Tf debug symbols are available.", 4)
            self._ui.showDebugFlags.setEnabled(False)
            return
    ...
    def _refreshCameraListAndMenu(self, preserveCurrCamera):
        ...
        if not preserveCurrCamera:
            ...
            # No explicit --camera arg: mirror Omniverse Kit's bound-camera
            # behaviour — probe the root layer's
            # `customLayerData["cameraSettings"]["boundCamera"]`. If that is
            # authored, switch to that camera. Otherwise leave cameraPrim
            # None so stageView falls back to its bbox auto-frame; we do
            # NOT auto-pick the first scene camera the way previous nanousd
            # workflows did because authored UsdGeomCamera prims are often
            # placeholders (e.g. OpenChessSet's wrapper authors a
            # placeholder camera the runtime overrides).
            if not self._startingPrimCameraName \
                    and not self._startingPrimCameraPath:
                bound_camera_path = None
                try:
                    root = self._dataModel.stage.GetRootLayer()
                    cs = (getattr(root, "customLayerData", None) or {}
                          ).get("cameraSettings", {}) if root else {}
                    bound_camera_path = cs.get("boundCamera") if cs else None
                except Exception:
                    bound_camera_path = None
    ...
    def _updatePropertyViewSelection(self):
        ...
        selectedPrimPropNames.update({
            prop.GetName(): {target for target in targets if target}
    ...
    def _updatePropertyViewInternal(self):
        ...
        for key, primProperty in self._propertiesDict.items():
            ...
            try:
                ...
                # nanousdview: skip property cells that raise (e.g. computed
                # attributes referencing un-bound _PseudoRootPrim members);
                # showing a blank value beats crashing the whole panel.
                valFunc = lambda: None
                attrText = ""
    ...
    def _setSelectedPrimsActivation(self, active):
        ...
        with BusyContext():
            ...
            for item in self.getSelectedItems():
                ...
                elif getattr(item, "isInPrototype", False):
