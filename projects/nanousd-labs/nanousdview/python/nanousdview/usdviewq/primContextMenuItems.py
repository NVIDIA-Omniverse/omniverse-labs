...
from .common import SetClipboardText, SpawnNanousdview
...
class IsolateCopyPrimMenuItem(PrimContextMenuItem):
    ...
    def RunCommand(self):
        ...
        subprocess.run([
            'usdcopy', '-inUsd', inFile,
            '-outUsd', outFile,
            '-sourcePath', str(focusPrim.GetPath()),
        ], check=False)
        SpawnNanousdview(outFile)
...
class IsolateAssetMenuItem(PrimContextMenuItem):
    ...
    def GetText(self):
        ...
        return "nanousdview asset%s" % name

    def RunCommand(self):
        print("Spawning nanousdview %s" % self._filePath)
        SpawnNanousdview(self._filePath)
