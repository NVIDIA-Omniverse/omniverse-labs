...
#define NU_MESHLET_STATS_VERSION 1u

typedef struct {
    uint32_t version;
    uint32_t struct_size;
    uint32_t active_meshlets;
    uint32_t active_meshlet_indices;
    uint32_t active_meshes_with_meshlets;
    uint32_t total_meshlets;
    uint32_t total_meshlet_indices;
    uint32_t meshlet_raster_enabled;
    uint32_t max_vertices;
    uint32_t max_triangles;
    uint64_t cpu_index_bytes;
...
typedef enum {
    ...
    NU_PIXEL_BGRA8     = 2,   /* 4 bytes per pixel, uint8 — B,G,R,A byte order.
                               * Note: Metal's color target is RGBA8 sRGB, so this
                               * format adds a CPU R↔B swizzle on readback (it
                               * does NOT save one as on Vulkan, where the target
                               * is natively BGRA). Provided for API parity. */
} NuPixelFormat;
...
typedef struct {
    ...
    int      visible;         /* Ignored by Metal; retained for shared ABI parity. */
} NuRendererConfig;
...
/* Set TLAS instance.mask bytes for meshes [0..count). Non-zero masks are
 * visible to Metal's current primary-ray mask; zero hides an instance from
 * RT. Pass NULL or count=0 to clear all masks back to 0xFF. */
NuResult     nu_set_instance_masks(NuRenderer* r,
                                   const unsigned char* masks,
                                   int count);

/* Vulkan-compatible per-environment partition API. Metal currently has no
 * per-camera cull-mask path, so this returns NU_ERROR_UNSUPPORTED rather than
 * silently rendering all envs together. */
...
/* Same as nu_load_usd_from_handle but lets the caller hint the
 * directory that should be scanned for sidecar .mtlx files (chess set
 * et al.). NULL/empty `scene_dir` falls back to the stage_label
...
/* Materialize geometry for a lazy-loaded scene. When NUSD_LAZY_MESH=1 is set
 * during nu_load_usd / nu_load_usd_from_handle, load returns a metadata-only
 * scene and parks the stage. This reruns eager extraction on that pinned stage.
 * Idempotent: returns NU_OK when there is no lazy load pending. */
NuResult     nu_extract_deferred(NuRenderer* r);

/* Materialize only lazy-loaded meshes whose world-space AABB intersects at
 * least one supplied camera frustum. vp_matrices is float[num_cameras * 16],
 * row-major view-projection matrices. Falls back to extract-all when there is
 * no pending lazy scene, no cameras, or no lazy AABB data. */
...
/* Serialize/load preprocessed geometry. The saved cache contains compact
 * renderer vertices, optimized indices, mesh records, optional meshoptimizer
 * meshlet data, and scene bounds. Loading a cache bypasses USD parsing and
 * extraction. MaterialX/textures are not serialized yet, so this is currently
 * a geometry-only cache for renderers created with enable_materials=0. */
...
/* Drop CPU-side geometry staging after GPU buffers/acceleration structures are
 * built. This is idempotent. Geometry mutation after finalize fails, but
 * transform/color/visibility updates still operate through mesh metadata and
 * GPU-side refits. */
NuResult     nu_finalize_scene(NuRenderer* r);

/* ---- Gaussian splat scene ----
 *
 * RT-only path for 3D Gaussian splats. Inputs match the AOUSD
 * UsdVol.ParticleField3DGaussianSplat schema shape: object-space centers,
 * linear sigma scales, wxyz orientations, post-sigmoid opacity, and SH
 * coefficients with DC at index 0. */
...
typedef struct {
    ...
    const float* orientations;     /* particle_count * 4 — wxyz quaternion */
    const float* opacities;        /* particle_count * 1 — [0,1] */
    const float* sh_coefficients;  /* particle_count * (sh_degree+1)^2 * 3 */
    int          sh_degree;        /* 0..3 */
    int          particle_count;
    const float* prim_xform;       /* optional row-major row-vector 4x4 */
} NuGsDesc;
 ...
 *
 * Returns NU_OK on success, NU_ERROR if r is NULL. */
 ...
 * column-vector convention; translation lives in the last column, i.e.
 * tx/ty/tz at flat indices 3/7/11. */
...
/* Set OVRTX/Omniverse-style camera exposure metadata. */
...
/* Vulkan-only CUDA export path. Metal exports this symbol for ABI parity and
 * returns NU_ERROR_UNSUPPORTED. */
...
/* Metal-only direct texture access (zero-copy alternative to nu_fetch_pixels
 * for callers that own a Metal device and want to read the renderer's color
 * + depth targets without a CPU round-trip).
 *
 * Returns id<MTLTexture> cast to void* (reference-bridged on the Metal side
 * of the renderer). Caller must NOT release; the texture is owned by the
 * renderer and lives until the next pipeline rebuild or nu_renderer_destroy.
 *
 * Returns NULL on the Vulkan/OpenGL backends (other backends have no Metal
 * texture to expose), and on the Metal backend before the first nu_render
 * call (the textures are lazily created with the swapchain dimensions).
 *
 * The depth target uses MTLPixelFormatDepth32Float — the value at each
 * pixel is the rasterizer-written normalized device coord depth in [0,1].
 * Useful for compositing UI overlays (gizmos, axes gizmo) that want
 * occlusion-aware shading without paying the CPU readback cost. */
...
/* Metal is headless; returns NULL. Present for shared backend ABI parity. */
...
/* Variant with explicit DomeLight intensity multiplier. Negative intensity
 * preserves the backend's default/scene-authored intensity behavior. */
NuResult     nu_load_environment_intensity(NuRenderer* r,
                                           const char* hdr_path,
                                           float intensity);

/* Runtime texture upload/bind entry points from the Vulkan backend. USD-authored
 * textures are supported by Metal's material loader; ad-hoc runtime texture
 * injection is not wired yet and returns NU_ERROR_UNSUPPORTED. */
...
/* Set the flat fallback DomeLight color used by Metal RT when no HDR IBL is
 * loaded. RGB is linear [0,1], intensity is a scalar multiplier. */
...
/* Vulkan/CUDA interop entry points. Metal exports them for shared ABI parity
 * and returns NU_ERROR_UNSUPPORTED. */
...
/* Deferred-shading toggles are Vulkan-only today. Metal accepts the calls as
 * no-ops so shared wrappers can keep one code path. */
void         nu_set_deferred_shade(NuRenderer* r, int enable);
void         nu_set_deferred_debug_mode(NuRenderer* r, uint32_t mode);

/* Vulkan/CUDA GPU-transform interop ABI. Metal returns NU_ERROR_UNSUPPORTED. */
typedef struct {
    int      mem_fd;
    uint64_t mem_size;
    int      count;
...
NuResult     nu_set_transform_layout(NuRenderer* r,
                                     const int* mesh_ids,
                                     int count);
...
/* Query meshoptimizer meshlet preprocessing state. Meshlets are generated when
 * NUSD_MESHLETS=1 or NUSD_MESHLET_RASTER=1 is set before renderer creation.
 * NUSD_MESHLET_RASTER=1 also renders raster meshes through the meshlet-derived
 * index stream. */
NuResult     nu_get_meshlet_stats(NuRenderer* r, NuMeshletStats* out_stats);

/* Cmd-buffer cache introspection. Metal records fresh command buffers today,
 * so all counters are currently zero. */
...
/* CPU-side phase timings for the Metal backend. Values are milliseconds and
 * are refreshed by the corresponding public calls. */
typedef struct {
    float rt_dispatch_ms;
    float pixel_readback_ms;
    float blas_build_ms;
    float tlas_build_ms;
    float curve_blas_build_ms;
    float staging_upload_segs_ms;
    float staging_upload_aabbs_ms;
    float staging_upload_colors_ms;
    float trace_rays_tiled_ms;
    float deferred_compute_ms;
} NuPhaseTimings;
