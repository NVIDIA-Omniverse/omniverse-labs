...
void camera_init(Camera* cam, const float bounds_min[3], const float bounds_max[3], float aspect)
{
    ...
    cam->yaw = 0.3f;
    cam->pitch = -0.3f;
    cam->fov_y = 45.0f * (3.14159265358979323846f / 180.0f);
    cam->aspect = aspect;
    cam->near_z = cam->distance * 0.01f;
    cam->far_z = cam->distance * 100.0f;
    cam->projection_shift_x = 0.0f;
    cam->projection_shift_y = 0.0f;
    cam->has_explicit_view = 0;
    memset(cam->view_matrix, 0, sizeof(cam->view_matrix));
}
...
void camera_get_proj(const Camera* cam, float out[16])
{
    ...
    out[0]  = f / cam->aspect;
    out[5]  = -f;                           /* Vulkan-style Y-flip in NDC.
                                             * RT's rgen uses proj_inv, which
                                             * inherits this sign and produces
                                             * correct rays for Metal NDC. The
                                             * raster vertex shader compensates
                                             * by negating clip-space Y after
                                             * the position multiply, so both
                                             * paths agree on the same camera. */
