...
/* Output: split layout — distances[num_rays] then normals[num_rays*3]
 * then hitPositions[num_rays*3].  distance = -1.0 means no hit. */
...
void main()
{
    ...
    /* Split output layout: distances[num_rays], normals[num_rays*3], hitPos[num_rays*3] */
    uint dist_off = idx;
    uint norm_off = num_rays + idx * 3;
    uint hit_off  = num_rays + num_rays * 3 + idx * 3;
    ...
    if (rayQueryGetIntersectionTypeEXT(rq, true) != gl_RayQueryCommittedIntersectionNoneEXT) {
         ...
         *   instanceCustomIndex  → mesh index in the SceneData SSBO
