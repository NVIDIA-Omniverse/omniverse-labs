...
"""Automated Metal-vs-OVRTX renderer-diff pipeline driven by NVIDIA ϜLIP.

Compares every Metal backend output (RT and raster) against its OVRTX golden using
...
Usage:
  ...
  python flip_pipeline.py --render        # re-render Metal frames first (needs assets)
...
BACKENDS = ("metal_rt", "metal_raster")  # tag -> golden is "<base>_ovrtx.png"
...
def _render(sets: tuple[str, ...]) -> None:
    """Re-render the Metal frames via the existing comparison harness (needs assets)."""
    for set_name in sets:
        print(f">>> re-rendering Metal frames: {set_name}")
        subprocess.run(
...
def write_report(records: list[dict[str, Any]], out_dir: Path, gated: bool,
    ...
    lines = ["# Metal vs OVRTX — ϜLIP diff report", ""]
    lines.append(
        "Each Metal backend output is compared to its OVRTX golden with the calibrated "
    ...
    if records:
        ...
        lines.append(
            f"**Systematic finding:** vs the OVRTX golden, {nb} Metal renders are brighter "
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Metal-vs-OVRTX FLIP diff pipeline")
    ...
    ap.add_argument("--render", action="store_true", help="re-render Metal frames first (needs assets)")
    ...
    if not records:
        ...
        print("SKIP: no OVRTX-golden/Metal frame pairs found under", frames_root)
        return 77
