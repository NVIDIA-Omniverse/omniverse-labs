...
class AttributeBinding:
    ...
    def map(self, device: Device = Device.CPU, device_id: int = 0) -> AttributeMapping:
        ...
        return self._renderer._map_bound_attribute(self, device)
...
def _render_var_arrays(
    ...
    out: dict[str, np.ndarray] = {}
    # Color-only fast path (port of nanousd-vulkan-renderer 0d349e5): an
    # interactive viewport requesting only LdrColor (no depth/normals/
    # segmentation) does not need the per-pixel mask, np.mgrid coordinate
    # grids, or fallback-distance arrays built below — that is tens of ms/frame
    # of numpy work whose result the LdrColor branch then discards (it just
    # returns `ldr`). Return it directly. Bit-identical to the full path for
    # names==["LdrColor"]; every other request falls through unchanged.
...
class Renderer:
    ...
    def _ensure_renderer(self):
        if self._nu is None:
            ...
            value = os.environ.get("NUSD_VIEW_ENABLE_MATERIALS")
            if value is not None:
                enable_materials = value.strip().lower() not in ("0", "false", "off", "no")
            value = os.environ.get("NUSD_DISABLE_MATERIALS")
            if value is not None and value.strip().lower() not in ("", "0", "false", "off", "no"):
...
"""
    ...
    def _load_inline_layer_text(
        ...
        if load_native:
            if prefix is None and sublayers:
                for layer_path in sublayers:
                    self._load_usd_path_recursive(layer_path, seen, path_prefix, load_native=True)
            else:
                source_path = self._write_runtime_layer("inline_source", usd_layer_content)
                load_path = source_path
                if prefix is not None:
                    wrapper = _make_reference_wrapper(source_path, prefix, usd_layer_content)
                    load_path = self._write_runtime_layer("inline_ref", wrapper)
                self._queue_native_load(load_path)

        if not (load_native and prefix is None and sublayers):
            for layer_path in sublayers:
