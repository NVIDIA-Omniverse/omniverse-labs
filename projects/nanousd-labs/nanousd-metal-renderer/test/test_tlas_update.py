...
CONVENTION NOTE: nu_set_transforms expects row-major row-vector matrices
(USD / GfMatrix4d style) — i.e. a 4x4 where the LAST ROW holds translation,
so flat indices 12/13/14 are tx/ty/tz and the bottom row reads (tx, ty, tz, 1).
This matches what scene.c writes when parsing USD. It is NOT the column-vector
convention typical of numpy/glm where translation lives at flat[3]/[7]/[11].
Earlier revisions of this file used column-vector convention and the tests
silently passed for tautological reasons (e.g. identity == identity after a
no-op move). All translations in this file are written at flat[12]/[13]/[14].
"""
...
class TestTLASUpdateCorrectness(unittest.TestCase):
    ...
    def test_move_object_changes_image(self):
        ...
        # Move red box to the right (row-vector convention: translation at flat[12])
        xform = np.eye(4, dtype=np.float32).flatten()
        xform[12] = 2.0  # translate X
    ...
    def test_many_update_cycles_no_drift(self):
        ...
        for i in range(500):
            ...
            xform[12] = math.sin(i * 0.1) * 3.0
            xform[13] = math.cos(i * 0.07) * 1.0
            r.set_transforms(mesh_ids=[0], transforms=xform.reshape(1, 16))
            _ = self._render_tiled(r)
    ...
    def test_multiple_objects_independent(self):
        ...
        xform = np.eye(4, dtype=np.float32).flatten()
        xform[12] = 100.0  # move far off-screen
        r.set_transforms(mesh_ids=[0], transforms=xform.reshape(1, 16))
    ...
    def test_all_meshes_update_simultaneously(self):
        ...
        for i in range(3):
            ...
            xforms[i, 13] = 2.0  # translate Y up (row-vector convention: flat[13])
...
class TestTLASUpdateKitchenSet(unittest.TestCase):
    ...
    def test_kitchen_move_restores(self):
        ...
        xform = np.eye(4, dtype=np.float32).flatten()
        xform[13] = 50.0  # move up (row-vector convention: flat[13])
