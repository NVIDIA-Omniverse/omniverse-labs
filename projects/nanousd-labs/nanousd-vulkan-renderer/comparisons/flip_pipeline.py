...
"""Automated Vulkan-vs-OVRTX renderer-diff pipeline driven by NVIDIA ϜLIP.

Compares every Vulkan backend output (RT and raster) against its OVRTX golden using
the calibrated FLIP metric in ``flip_compare`` and produces:

  * a per-pair FLIP heatmap strip (``golden | vk | FLIP``) under ``flip/``,
...
Usage:
  ...
  python flip_pipeline.py --render        # re-render Vulkan frames first (needs assets)
...
RENDERER = "Vulkan"
BACKENDS = ("vk_rt", "vk_raster")  # tag -> golden is "<base>_ovrtx.png"
...
def _render(sets: tuple[str, ...]) -> None:
    """Re-render the Vulkan frames via the existing comparison harness (needs assets)."""
    for set_name in sets:
        print(f">>> re-rendering Vulkan frames: {set_name}")
        subprocess.run(
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Vulkan-vs-OVRTX FLIP diff pipeline")
    ...
    ap.add_argument("--render", action="store_true", help="re-render Vulkan frames first (needs assets)")
