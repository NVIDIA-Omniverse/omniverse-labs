...
/* Splice all blocks from `src` onto the tail of `dst` and clear `src`.
 * After this call, pointers handed out from `src` remain valid (the
 * blocks themselves move ownership but are never freed/relocated), and
 * `dst` becomes the sole owner. `src` is left empty (head/current=NULL)
 * so a subsequent arena_destroy(src) is a no-op.
 *
 * Used to merge per-thread arenas back into the main arena after a
 * parallel curve-load region. NOT thread-safe — must be called from the
 * serial portion. */
static inline void arena_splice(Arena* dst, Arena* src) {
    if (!dst || !src || !src->head) return;
    if (!dst->head) {
        dst->head = src->head;
        dst->current = src->current;
        if (src->default_block_size > dst->default_block_size)
            dst->default_block_size = src->default_block_size;
    } else {
        /* Walk to the last block of dst (current may not be the tail if
         * an earlier splice extended past it). */
        ArenaBlock_* tail = dst->current ? dst->current : dst->head;
        while (tail->next) tail = tail->next;
        tail->next = src->head;
        dst->current = src->current;  /* future allocs land in src's tail */
    }
    src->head = NULL;
    src->current = NULL;
}
