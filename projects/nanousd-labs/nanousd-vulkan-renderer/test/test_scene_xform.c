...
/*
 * test_scene_xform.c — CPU scene-loader transform correctness checks.
 */
...
static int check_point_instancer_compact(const char* path)
{
    setenv("NUSD_RENDER_PI_BATCHES", "1", 1);
    Scene* scene = scene_load(path);
    unsetenv("NUSD_RENDER_PI_BATCHES");
    if (!scene) {
        fprintf(stderr, "FAIL: scene_load returned NULL for %s\n", path);
        return 1;
    }
    ...
    int fail = 0;
    if (scene->nmeshes != 1) {
        fprintf(stderr, "FAIL: compact PI expected 1 prototype mesh, got %d\n",
                scene->nmeshes);
        fail = 1;
    } else if (!scene->meshes[0].is_proto_only) {
        fprintf(stderr, "FAIL: compact PI prototype mesh is not marked proto-only\n");
        fail = 1;
    }
    if (scene->npi_batches != 1 || scene->npi_transforms != 3) {
        fprintf(stderr,
                "FAIL: compact PI expected 1 batch/3 transforms, got %d/%llu\n",
                scene->npi_batches,
                (unsigned long long)scene->npi_transforms);
        fail = 1;
    }
    if (!nearf4(scene->bounds_min[0], -5.0f) ||
        !nearf4(scene->bounds_max[0],  5.0f) ||
        !nearf4(scene->bounds_min[1], -1.0f) ||
        !nearf4(scene->bounds_max[1],  1.0f)) {
        fprintf(stderr,
                "FAIL: compact PI bounds = "
                "(%.3f, %.3f, %.3f) - (%.3f, %.3f, %.3f), "
                "expected x=[-5,5] y=[-1,1]\n",
                scene->bounds_min[0], scene->bounds_min[1], scene->bounds_min[2],
                scene->bounds_max[0], scene->bounds_max[1], scene->bounds_max[2]);
        fail = 1;
    }
...
int main(int argc, char** argv)
{
    ...
    fail |= check_point_instancer_compact(argv[4]);
    if (fail) return 1;
    printf("PASS: scene transform, metrics, UV seam, and compact PI checks\n");
    return 0;
}
